...
function M.shell()
  vim.ui.input({ prompt = '$ task: ' }, function(task)
    ...
    local cwd = require('workspace.repos').active_cwd()
    run(table.concat({
...
function M.docs()
  ...
  run(table.concat(parts, '\n'), 'writing docs…', function(result)
    ...
    util.preview_float(doc, ' Claude Docs ', function()
      ...
      vim.notify('workspace: docs inserted', vim.log.levels.INFO)
    end)
...
local function apply_changes(changes)
  ...
  local function next_file()
    if idx > #paths then
      vim.notify('workspace: composer done', vim.log.levels.INFO); return
    end
    local rel      = paths[idx]
...
function M.compose()
  ...
  if #bufs == 0 then
    vim.notify('workspace: no open file buffers', vim.log.levels.WARN); return
  end
