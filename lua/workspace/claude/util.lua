...
-- Put the last line at the bottom of the window (tail view). Shared by the
-- read-only float, the streaming float, and the live transcript viewer.
...
-- Backed by workspace.claude.runner (structured stream-json): no shell string,
...
function M.run(prompt, label, callback, opts)
  ...
  require('workspace.claude.runner').run {
    prompt = prompt,
    model  = opts.model,
    cwd    = opts.cwd,
    label  = label,
    on_result = function(res)
      ...
      if res.is_error then
        vim.notify('workspace: ' .. (res.text or 'Claude error'), vim.log.levels.ERROR)
        return
      end
    ...
    on_error = function(msg)
      stop()
      vim.notify('workspace: ' .. msg, vim.log.levels.ERROR)
    end,
  }
end
...
function M.read_float(lines, title, ft, opts)
  ...
  local width  = opts.width  or math.min(math.floor(vim.o.columns * 0.72), 110)
  ...
  if opts.at_end then M.scroll_to_bottom(win) end
