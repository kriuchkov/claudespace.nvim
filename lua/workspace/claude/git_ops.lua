...
local run = require('workspace.claude.util').run
...
local function generate_commit_message()
  ...
  if vim.v.shell_error ~= 0 then
    vim.notify('workspace: not in a git repository', vim.log.levels.ERROR); return
  end
  if vim.trim(diff) == '' then
    vim.notify('workspace: no staged changes', vim.log.levels.WARN); return
  end
  ...
  run(table.concat({
    ...
    for _, buf in ipairs(vim.api.nvim_list_bufs()) do
      if vim.bo[buf].filetype == 'gitcommit' then
        ...
        vim.notify('workspace: commit message inserted', vim.log.levels.INFO)
        return
      end
    end
    vim.fn.setreg('+', msg)
    vim.notify('workspace: commit message copied to clipboard:\n' .. msg, vim.log.levels.INFO)
  end)
end
...
local function generate_pr()
  ...
  if vim.v.shell_error ~= 0 or branch == '' then
    vim.notify('workspace: not in a git repo or on a detached HEAD', vim.log.levels.WARN); return
  end
  if branch == base then
    vim.notify('workspace: not on a feature branch', vim.log.levels.WARN); return
  end

  local log  = vim.fn.system('git log ' .. base .. '..HEAD --oneline 2>/dev/null')
  local diff = vim.fn.system('git diff ' .. base .. '...HEAD --stat 2>/dev/null')
  if log == '' then
    vim.notify('workspace: no commits ahead of ' .. base, vim.log.levels.WARN); return
  end
  ...
  run(table.concat({
    ...
    vim.keymap.set('n', '<leader>y', function()
      vim.fn.setreg('+', title .. '\n\n' .. body)
      vim.notify('workspace: PR description copied to clipboard', vim.log.levels.INFO)
...
local function review(lines, ft, context)
  if #lines == 0 then
    vim.notify('workspace: buffer is empty', vim.log.levels.WARN); return
  end
  ...
  run(table.concat({
    ...
    if #qf > 0 then
      vim.fn.setqflist(qf)
      vim.notify('workspace: ' .. #qf .. ' findings in quickfix (<leader>xq)', vim.log.levels.INFO)
    end
  end)
end
