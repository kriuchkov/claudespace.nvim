...
function M.fix()
  ...
  if #diags == 0 then
    vim.notify('workspace: no diagnostics', vim.log.levels.WARN); return
  end
  ...
  run(table.concat({
    ...
    util.preview_float(preview, ' Claude Fix ', function()
      ...
      vim.notify('workspace: fix applied', vim.log.levels.INFO)
    end)
  end)
end
...
local function navic_scope(bufnr)
  local ok, nav = pcall(require, 'workspace.navic')
