...
local WDIR = fn.stdpath('data') .. '/workspace_workspaces'

-- One-time migration from the pre-rename data dir so saved workspaces survive.
do
  local old = fn.stdpath('data') .. '/claudespace_workspaces'
  if fn.isdirectory(old) == 1 and fn.isdirectory(WDIR) == 0 then
    pcall(fn.rename, old, WDIR)
  end
end
...
function M.save(name, silent)
  ...
  -- Save window layout
  local layout
  local ok_lay, lay = pcall(require, 'workspace.layout')
  ...
  local state = {
    ...
    layout = layout, root = ws_root, active_repo = active_repo,
  }
...
function M.load(name)
  ...
  pcall(function() require('workspace.repos').load() end)
  ...
  vim.schedule(function()
    pcall(function()
      require('workspace.tabline').reapply_groups()
    end)
  end)
