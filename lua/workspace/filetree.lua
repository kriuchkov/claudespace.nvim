...
local function repo_name_hl(m, active)
  ...
  local st = require('workspace.repos').status(m)
...
-- Route files into the shell's single center window. Delegating here (instead of
-- a private editor-window search) keeps one source of truth for "the editor":
-- a private search with narrower criteria used to miss a center holding a
-- terminal / start screen and split a second editor beside it.
...
local function repo_info()
  local e = entry_at_cursor()
  require('workspace.repos').show_info(e and e.path or S.root)
end
...
local function create_buf()
  ...
  k('n', '?',     show_help,         o)
  -- The tree is wrap=false, so a horizontal mouse wheel shifts the whole tree
  -- sideways (names slide out of view) — never wanted in a fixed-width panel.
  k('n', '<ScrollWheelLeft>',  '<Nop>', o)
  k('n', '<ScrollWheelRight>', '<Nop>', o)
  return buf
end
